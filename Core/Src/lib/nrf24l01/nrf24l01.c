...
{
	return nrf24l01_port_pin_irq_active();
}
...
void nrf24l01_spi_send_read(unsigned char * data, unsigned int len, bool copydata)
{
	...
	{
		if(copydata != false)
			data[count] = spi_port_send_read_byte(data[count]);
		else
		{
			tempbyte = data[count];
			spi_port_send_read_byte(tempbyte);
		}
	}
}
